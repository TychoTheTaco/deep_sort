...
def generate_detections(encoder, image_dir, detections_path, output_dir):
    ...
    for item in detections_json:
        for d in detections_json[item]:
            # <frame>, <id>, <bb_left>, <bb_top>, <bb_width>, <bb_height>, <conf>, <x>, <y>, <z>
            data.append((int(item), -1, d['x1'], d['y1'], d['x2'] - d['x1'], d['y2'] - d['y1'], d['score'], -1, -1, -1))

    detections_in = np.array(sorted(data, key=lambda x: x[0]))
    detections_out = []
    ...
    np.save(str(Path(output_dir, '../detections.npy')), np.asarray(detections_out), allow_pickle=False)
...
def main():
    ...
    generate_detections(encoder, args.image_dir, args.detections_path, '.')
